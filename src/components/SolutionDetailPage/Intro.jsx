...
const Intro = () => {
    ...
    return (
        <section data-theme="orange" className="bg-gradient h-[75vw] relative max-sm:h-fit w-screen overflow-hidden">
            <div className="px-[4vw] py-[8vw] flex flex-col justify-between items-center h-full w-full relative max-sm:items-start max-sm:px-[5.5vw] max-sm:py-[15%]">
                <div className="text-center text-white w-[80%] flex flex-col items-center gap-10 max-sm:w-full max-sm:text-left max-sm:items-start">
                    ...
                    <Copy>
                        <p className="text-content-20 w-9/12 max-sm:w-[95%]">Enable customers to open accounts, make transfers, pay bills, and top up airtime directly from your app. Moneylink’s SDK simplifies front-end integration while ensuring secure, fast back-end processing.</p>
                    </Copy>
                </div>
                 <div className="flex flex-row flex-wrap  items-center justify-between w-[50%] gap-y-[5vw] max-sm:w-full max-sm:flex-col max-sm:items-start max-sm:mt-[15vw] max-sm:mb-[20vw]">
                                    {uspData.map(({ description, svg, svgElement }, index) => (
                                        <div key={index} className="flex items-center w-[40%] justify-start gap-[1vw] max-sm:w-[80%] max-sm:gap-[7vw]">
                                            <div className="text-primary-2 mb-1 w-fit">
                                            <svg width="105" height="85" viewBox="0 0 105 85" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-[6vw] w-[6vw] max-sm:w-[18vw] max-sm:h-[18vw]" ref={svg}>
                                                    {svgElement}
                                                </svg>
                                            </div>
                                            <Copy>
