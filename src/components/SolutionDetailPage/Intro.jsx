...
const Intro = ({introData}) => {
    ...
    const uspData = [
        {
            description: `${introData.uspData[0].description}`,
            width: `${introData.uspData[0].width}`,
            svg: svgRefs[0],
            svgElement: <DigitalAccounts1/>
        },
        {
            description: `${introData.uspData[1].description}`,
            width: `${introData.uspData[1].width}`,
            svg: svgRefs[1],
            svgElement: <DigitalAccounts2/>
        },
        {
            description: `${introData.uspData[2].description}`,
            width: `${introData.uspData[2].width}`,
            svg: svgRefs[2],
            svgElement: <DigitalAccounts3/>
        },
        {
            description: `${introData.uspData[3].description}`,
            width: `${introData.uspData[3].width}`,
            svg: svgRefs[3],
        ...
        <section data-theme="orange" className="bg-gradient h-[85vw] max-md:h-fit relative max-sm:h-fit w-screen overflow-hidden">
            <div className="px-[4vw] py-[8vw] flex flex-col justify-between items-center h-full w-full relative max-sm:items-start max-sm:px-[5.5vw] max-sm:py-[15%] max-md:py-[15%]">
                <div className="text-center text-white w-[80%] flex flex-col items-center gap-10 max-sm:w-full max-sm:text-left max-sm:items-start">
                    <Copy>
                        <h2 className="text-head-100 font-display">{introData.heading}</h2>
                    </Copy>
                    <Copy>
                        <p className="text-content-20 w-9/12 max-md:w-[100%] max-sm:w-[95%]">{introData.para}</p>
                    </Copy>
                </div>
                ...
                <div className="text-center w-[85%] max-sm:text-left max-sm:w-full ">
                    <Copy>
                    <p className="text-head-60 max-sm:text-head-60 max-md:text-[4.5vw] max-sm:pt-0 max-md:pt-[4vw] font-display ">{introData.headline}</p>
                    </Copy>
                </div>
